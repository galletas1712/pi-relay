...
- End your final message with a single line `suggested_next: <value>` (e.g. `pass`, `bugs_found`, or `environment_issue`), choosing the outcome the orchestrating task lists, so the parent can branch on it from the handoff index.json.
