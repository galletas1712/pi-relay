...
- End your final message with a single line `outcome: <value>` (e.g. `approved` or `changes_requested`), choosing the outcome the orchestrating task lists, so the parent can branch on it from the delivered delegation snapshot, or from a refreshed `inspect_delegation` snapshot.
