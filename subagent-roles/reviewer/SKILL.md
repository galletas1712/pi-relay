...
- End your final message with a single line `suggested_next: <value>` (e.g. `approved` or `changes_requested`), choosing the outcome the orchestrating task lists, so the parent can branch on it from `inspect_delegation`.
