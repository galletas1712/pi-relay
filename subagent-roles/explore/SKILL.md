...
- Prefer cheap, scoped searches first (`rg` with tight paths) before reading large ranges.
...
- When the orchestrating task asks for a typed outcome, end your final message with a single line `outcome: <value>` (e.g. `done` or `inconclusive`) so the parent can branch on it from the delivered delegation snapshot, or from a refreshed `inspect_delegation` snapshot.
