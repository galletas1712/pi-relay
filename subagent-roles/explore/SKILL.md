...
- When the orchestrating task asks for a typed outcome, end your final message with a single line `suggested_next: <value>` (e.g. `done` or `inconclusive`) so the parent can branch on it from `inspect_delegation`.
