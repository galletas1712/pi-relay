// Autonomous mode for prime-autonomy (M4, G3).
//
// PROVENANCE: ported from prime-agent
//   packages/coding-agent/src/core/autonomous.ts (runtime state, limits, gates,
//   git-worktree snapshot skip-if-unchanged, gate failure continuation text).
// Extension-land adaptations:
// - No PA shell utils (trackDetachedChildPid / killProcessTree): gates spawn
//   detached process groups and kill the group on timeout/abort.
// - The continuation is produced as plain text at agent_settled; index.ts
//   delivers it via pi.sendUserMessage (PA injects a UserMessage directly).
// - Usage accounting is driven by index.ts from pi message_end events
//   (addAutonomousUsage per assistant message, PA parity).
...
export function addAutonomousUsage(state: AutonomousRuntimeState, usage: UsageLike | undefined): void {
	if (!state.enabled) {
		return;
	}
	state.turnsUsed++;
...
function autonomousTokenDelta(usage: UsageLike | undefined): number {
	if (!usage) {
		return 0;
	}
...
/** PA nextAutonomousContinuation, returning prompt TEXT for pi.sendUserMessage. */
export async function nextAutonomousContinuation(
	...
	state.continuationsUsed++;
	return decision.reason === "gate_failed"
...
function runChildProcess(
	...
	return new Promise((resolvePromise) => {
		...
		const killChild = () => {
			// Detached group kill (PA uses killProcessTree; -pid kills the group).
			try {
		...
		const finish = (result: Pick<ChildProcessResult, "status" | "signal">) => {
			...
			resolvePromise({ ...result, stdout, stderr, error, timedOut, outputTruncated });
		};
		...
		child.on("error", (err) => {
			error = err;
			finish({ status: child.exitCode, signal: null });
		});
