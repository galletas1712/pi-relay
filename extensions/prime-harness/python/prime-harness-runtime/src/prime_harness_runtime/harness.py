"""Persistent harness-state helpers for the pi-relay prime-harness extension (M2).

PROVENANCE: near-verbatim port of prime-agent's
  prime-agent-runtime/src/rlm/harness.py
(upstream repo: repos/prime-agent, module rlm.harness). The state model and the
harness_state.json file schema are identical so harness state is interchangeable
between prime-agent and this extension. M2 adaptations:
  - module docstring only; code below is the upstream file unchanged.
...
directory by default. Execution still belongs to the TypeScript host
and the existing ``rlm.run`` recursion bridge.

M7 adds a fifth entry kind, ``role`` (packaged subagent roles, pi-relay
SkillKind::SubagentRole as harness content) — additive and tolerated as
unknown by upstream PA loaders, so state files stay interchangeable.
"""
...
# M7: "role" is an additive extension over PA's four kinds (pi-relay
# SkillKind::SubagentRole as harness content). The loader ignores unknown/missing
# kind keys both here and in PA, so state files stay interchangeable. Role entry
# content is the full SKILL.md role document; entry id must equal the
# frontmatter name (pi-relay's directory-name doctrine).
HarnessKind = Literal["prompt", "memory", "skill", "subagent", "role"]
HarnessScope = Literal["local", "global"]

_DEFAULT_FILE_NAME = "harness_state.json"
_DEFAULT_HARNESS_DIR_NAME = "harness"
_KINDS: tuple[HarnessKind, ...] = ("prompt", "memory", "skill", "subagent", "role")
...
class HarnessState:
    ...
    def create_role(
        ...
        **kwargs: Any,
        ...
        """Create a packaged subagent role (M7; pi-relay SkillKind::SubagentRole).

        ``content`` is the full SKILL.md role document (frontmatter ``name``,
        ``description``, optional ``model``/``reasoning_effort``/``max_tokens``/
        ``skills`` preload list, then the body). The entry id MUST equal the
        frontmatter ``name`` — pass ``id="<name>"`` or give ``title`` the role
        name so the derived slug matches. Spawn with ``await rlm("...", role="<name>")``.
        """
        return self.create("role", title, content, id=id, path=path, metadata=metadata, global_=global_, **kwargs)
    ...
    def update_role(
        ...
        **kwargs: Any,
        ...
        return self.update("role", id, title, content, path=path, metadata=metadata, global_=global_, **kwargs)

    def delete_role(self, id: str, *, global_: bool = False, **kwargs: Any) -> bool:
        return self.delete("role", id, global_=global_, **kwargs)
