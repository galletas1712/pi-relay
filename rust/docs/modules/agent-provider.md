...
arguments. OpenAI custom `apply_patch` output is normalized to
`{"input": ...}` while preserving the raw custom replay and result kinds. The
daemon's main-agent completion wrapper validates only newly returned canonical
`Bash` calls; all other first-party, MCP, and provider-owned calls retain their
...
`GET /models?client_version=0.153.4`. The GET uses the same bearer/account,
...
- `ReasoningEffort::default()` is `Medium`. OpenAI exact-validates the known
  ...
  metadata determines whether the selected `low…max` effort is emitted. Opus
  5, Sonnet 5, Fable 5.1, and Opus 4.7 support that range.
- Claude Fable 5.1 is intentionally an explicit opt-in UI choice: Anthropic requires 30-day data retention for Fable and does not offer Zero Data Retention for it. Do not silently select Fable for a ZDR workload.
