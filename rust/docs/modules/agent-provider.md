...
`ModelResponse` = `{ assistant: AssistantMessage, provider_replay, usage: Option<ProviderUsage>, stop_reason, stop_details }`. `ModelStopReason` is `Complete`, `MaxOutputTokens`, or `Refusal`. Refusal details retain the optional provider category and human-readable explanation.
...
`ProviderError` variants: `Http`, `Timeout`, `Transient`, `Provider`, `Status { status, message }`, `Json`. The daemon's model-dispatch loop retries every `ProviderError` up to five attempts; the provider crate does not classify status codes as retryable or non-retryable.
