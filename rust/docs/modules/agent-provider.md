...
- `ReasoningEffort::default()` is `Medium`. OpenAI accepts `Max` only for `gpt-5.6-sol`; other gpt-5.x models clamp it to `xhigh`. Anthropic rejects `None`/`Minimal`; it maps `XHigh` down to `high` unless the model is opus-4-7/4-8.
