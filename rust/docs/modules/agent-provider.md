...
Both provider adapters implement `compact`. Anthropic also validates the
selected model before constructing the compact request; adapter support alone
does not imply that every Claude model supports native compaction.
