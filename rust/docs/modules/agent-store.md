...
selected-subagent control and an attempt-fenced post-compaction dispatch intent
in `pending` or `running`, described below. The latter's payload contains the
narrower durable owner/generation/expiration lease used for restart reclaim.
Rows actually made stale have any post-compaction marker removed. Ordinary
...
- `transcript_leaf_is_turn_boundary` / `active_leaf_is_turn_boundary` —
  ...
  boundary). `history.switch` applies it to the requested target and
  `history.fork` to the source's current active leaf, so a forked child never
  starts mid-turn. Both write transactions hold the source session row lock
  while rejecting running delegations; delegation creation takes the same
