...
- While implementing, the active-branch sync helper was kept inside `refreshSelectedSessionState` rather than as a separate callback. That avoids a React callback dependency cycle (`getFreshSession` <-> sync helper) and keeps the reconciliation entrypoint singular: callers ask to refresh, and the refresh function decides incremental-vs-full from the current cache.
- `history.switch` validates both `expected_transcript_revision` and the compact-tree-derived branch ID list before returning sparse bodies. The revision check catches any transcript append between `/switch` derivation and mutation; the branch-ID check catches display-parent/compaction topology disagreement without requiring a full body payload.
- The branch ID validation and sparse body query are performed inside the same Postgres transaction as the active-leaf update/event insert. That avoids a post-commit race where a new append could make the returned branch projection newer than the switch event.
- The store implementation computes branch IDs first and only materializes bodies by ID when the caller asks for full or sparse bodies. That keeps the common sparse-switch path small while preserving the legacy full-branch option.
