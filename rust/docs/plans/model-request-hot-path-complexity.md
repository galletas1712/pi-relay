...
`a59a447f1f3eb6712a1c4efb9aada8650b5d0915` (#227), not on the older local
`HEAD` or the large uncommitted streaming/eager-tool prototype in the working
tree.
...
- `agent-provider/src/sse.rs`:
  bytes received, delimiter scan windows, frames, peak retained bytes.
...
- Focused deterministic complexity probes under the owning Rust crates and a
  loopback mock-provider aggregate; do not add a live-provider harness.

**Acceptance**

- A deterministic long-context fixture covers at least 1, 10, and 100 MiB
  logical contexts without contacting a provider.
- SSE fixtures cover one-byte chunks, an explicitly ignored 1 MiB fragmented
  frame, and a worst-backlog shape with 1,000 small frames.
- Action benchmarks cover K = 1, 10, 100, and 1,000 reverse-order
  completions.
- One ordinary no-tool turn through a loopback provider reports exact logical
  builds, serializations/compressions, physical sends, SSE scan windows,
  scoped output SQL/transactions/store calls, completion persistence, idle
  transition, and registry scans.
- Context materialization bytes and four named B-sized clone sites are
  deterministic content-size probes; no counter claims allocator-exact bytes.
- Cold activation reports rows/bytes loaded and recovery SQL separately from
  the warm turn.
- Disabled instrumentation has a paired 21-sample statistical report for O(1),
  SSE, and reverse-K shapes with no flaky wall-clock threshold. The K/SSE
  implementation preserves the original loop paths when disabled.

Stage 0's SQL limits are explicit: `output_sql_statements` covers scoped output
persistence plus the idle-event insert, while `recovery_sql_statements` covers
the two `load_stored_session` queries. `scoped_store_calls` is a high-level call
diagnostic, not a statement estimate. A later stage that needs a total SQL
target must first add dedicated total-query measurement; it must not infer a
total from these counters.
