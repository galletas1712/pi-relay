...
**Model IDs (model_metadata.rs):** OpenAI `gpt-5.6-sol, gpt-5.6-terra, gpt-5.6-luna, gpt-5.5, gpt-5.1,
...
4. **Start the e2e session WITHOUT the harness flag** (omit `metadata.harness` or set false) so real dispatch runs. Pick
   model IDs from model_metadata.rs — e.g. OpenAI `gpt-5.6-sol`, Claude `claude-sonnet-5`/`claude-opus-4-8`.
