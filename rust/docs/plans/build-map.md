...
| rust/crates/agent-daemon/src/config.rs | Config::from_env_and_args | startup policy | Reads required XDG `pi-relay/agentd/config.toml` root `database_url`, optional `bind` (default `127.0.0.1:8787`), and model policy. The daemon accepts no configuration arguments. |
