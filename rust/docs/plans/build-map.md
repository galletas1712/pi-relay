...
| rust/crates/agent-daemon/src/config.rs | Config::from_env_and_args | startup policy | Reads required XDG `pi-agentd/config.toml` root `database_url`, optional `bind` (default `127.0.0.1:8787`), and model policy. The daemon accepts no configuration arguments. |
...
1. **Launch:** create XDG `pi-agentd/config.toml` with `database_url` (and optional `bind`), then run `pi-agentd` with no arguments. `store.migrate()` fully
