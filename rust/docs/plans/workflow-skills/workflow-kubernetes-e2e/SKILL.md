...
description: Run an adaptive end-to-end test against a Kubernetes cluster as a single full stage, with cluster-safety rules. Use for operator/e2e validation where the agent must observe rollout and classify the outcome.
---

# Workflow: kubernetes e2e

Run an adaptive end-to-end test against a Kubernetes cluster as a single full
stage. The subagent deploys/observes/collects evidence and classifies the
result. You decide what to do with that result.

## Stages
- kubernetes-tester — full subagent (runs cluster ops; writes evidence files
  into the workspace and reports their paths).

## Outcomes (suggested_next, in index.json)
- kubernetes-tester: pass | product_failure | environment_retry | human_needed
...
- test: delegate_writing_task({ role: "kubernetes-tester",
    prompt: "<context, namespace, what to deploy/test, what 'pass' means>",
    workflow: "kubernetes_e2e" })

Cluster-safety rules to put in the tester's prompt (and the role SKILL.md):
...
- Collect logs/events/manifests as evidence files in the workspace and list
  their paths in the final message (these persist because it is a full stage).
