...
| `agent-mcp` | Operator-configured stdio/Streamable HTTP MCP clients, Codex-parity rmcp-backed OAuth with daemon-owned permission-protected file credentials, restart restoration/refresh, bounded bearer injection, sanitized internal status/local logout, New Session inventory, and deterministic MCP-only session manifests kept separate from `ToolRegistry`. Public OAuth RPC/UI remain Stage 4. | [plans/mcp-client.md](plans/mcp-client.md) |
