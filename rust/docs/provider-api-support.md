...
| Capability | OpenAI private Codex adapter | Anthropic Messages adapter | Evidence and limitations |
| --- | --- | --- | --- |
| HTTP SSE generation | **Supported.** Reconciles added and completed output-item lifecycles when added events are present, and reconciles terminal `response.output` when present; output indices must be unique and contiguous and determine replay order. Done-only private streams remain supported. | **Supported.** Uses a bounded content-block state machine with checked contiguous indices, required block stops, strict known deltas, and malformed accumulated JSON failure. | Shared SSE framing handles chunk/frame mechanics only. Historical ordinary turns exist for both providers. `[source, unit, live]` |
