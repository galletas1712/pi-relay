...
User-Agent. The minimum association between that release and Astra first came from
external release evidence inspected locally. Initial authenticated catalog GETs
for this account at versions 0.147.0, 0.149.1, 0.153.4, and 0.154.0 omitted
exact slug `gpt-6-astra`, and a direct inference attempt returned HTTP 400:
`The 'gpt-6-astra' model is not supported when using Codex with a ChatGPT
account.` A later authenticated GET at the pinned 0.153.4 identity returned
eight models including exact slug `gpt-6-astra`. Its entry reports a 272k
current/872k maximum context window, efforts `low` through `ultra`, reasoning
summaries, parallel tool calls, and Responses Lite. A repository-adapter
`high`-effort request then returned `Complete` with exact requested text
`probe-ok` and usage of 33 input/6 output tokens.

The picker seeds the catalog-supported public subset `low…max` and initial
effort `high`. An ordinary or compact request still fails locally before its
generation POST whenever the exact catalog entry is absent. No alias or
fallback is used. The GPT-5.6 family
...
| Capability | OpenAI private Codex adapter | Anthropic Messages adapter | Evidence and limitations |
| --- | --- | --- | --- |
| Function / custom tools | **Supported.** JSON function calls and free-form custom calls become normalized local tool calls; results replay in matching output forms. Newly returned canonical `Bash` calls require a short `call_description`; other first-party and MCP calls retain their own contracts. | **Supported.** JSON `tool_use` calls and results become normalized local tool calls/results. Newly returned canonical `Bash` calls require a short `call_description`; other first-party and MCP calls retain their own contracts. | Local Bash uses this relay-owned description contract; LoadSkill, web wrappers, delegation tools, provider-shaped edit declarations, and MCP retain their own schemas. `[source, unit, live]` |
| Shell and apply-patch / text editor | **Partial.** pi-relay offers Bash as a function whose JSON schema requires `call_description`, and `apply_patch` as a raw custom free-form tool. Native `local_shell_call`, `shell_call`, and `apply_patch_call` output actions fail closed instead of being mistaken for passive output. | **Partial.** Bash is a local JSON client tool whose schema requires `call_description`; Edit retains the native `text_editor_20250728` declaration named `str_replace_based_edit_tool`. Other native shell action families are not implemented. | Both local edit paths have unit coverage; Anthropic Bash/editor and OpenAI Bash have historical live coverage. `[source, unit, live]` |
