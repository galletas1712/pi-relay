...
| Vision / image *input* | **Supported.** Resolved user and tool-result artifact refs map to Responses `input_image` data URLs. Text-only tool results keep string `function_call_output`; multimodal results use an ordered `input_text`/`input_image` array. Missing resolved refs fail before request send. | **Supported.** Resolved user and tool-result artifact refs map to ordered Messages base64 image sources; missing refs fail before request construction. | Durable content is only `{"type":"image","artifact_id":"sha256:…"}`. The daemon batch-loads and integrity-checks bytes; base64/data URLs are transient provider transport. There is no production URL source. `[source, unit]` |
