...
| Capability | OpenAI private Codex adapter | Anthropic Messages adapter | Evidence and limitations |
| --- | --- | --- | --- |
| Function / custom tools | **Supported.** JSON function calls and free-form custom calls become normalized local tool calls; results replay in matching output forms. New main-agent first-party calls require a short `call_description`. | **Supported.** JSON `tool_use` calls and results become normalized local tool calls/results. New main-agent first-party calls require a short `call_description`. | Local Bash, LoadSkill, web wrappers, delegation tools, and provider-shaped edit declarations use this contract. Externally defined MCP tools retain server-owned contracts and are excluded for now. `[source, unit, live]` |
| Shell and apply-patch / text editor | **Partial.** pi-relay offers Bash as a function and `apply_patch` as a custom free-form tool whose grammar begins with a required description header. Native `local_shell_call`, `shell_call`, and `apply_patch_call` output actions fail closed instead of being mistaken for passive output. | **Partial.** Bash and Edit are local JSON client tools; Edit retains the `str_replace_based_edit_tool` operation schema because Anthropic's native editor declaration cannot carry `call_description`. Other native shell action families are not implemented. | Both local edit paths have unit coverage; Anthropic Bash/editor and OpenAI Bash have historical live coverage. `[source, unit, live]` |
