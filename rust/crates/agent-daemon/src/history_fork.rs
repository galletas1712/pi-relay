...
use crate::history::ensure_history_source_idle;
...
pub(crate) async fn fork(state: &AppState, params: Value) -> Result<Value, RpcError> {
    let ForkParams { session_id } = from_params(params)?;
    let driver = SessionDriver::acquire(state, &session_id).await;
    ensure_history_source_idle(state, &driver, &session_id).await?;
    ...
    config.metadata = fork_metadata(config.metadata, &session_id);
...
fn fork_metadata(mut metadata: Value, source_session_id: &str) -> Value {
