...
mod tests {
    ...
    fn role_resolves_provider_and_home_global_preloads() {
        ...
        assert_eq!(resolved.context, RoleContextPolicy::Forked);
    }

    #[test]
    fn role_context_policy_supports_explicit_values_and_rejects_invalid_frontmatter() {
        for (frontmatter, expected) in [
            ...
            assert_eq!(
                resolve_skill_role(&[role], "reviewer")
                    .expect("valid role context")
                    .context,
                expected
            );
        }

        for invalid_context in ["inherited", "[forked]"] {
            let role = raw_skill(
                SkillKind::SubagentRole,
                SkillOrigin::RuntimeRole,
                None,
                "reviewer",
                &format!("context: {invalid_context}\n"),
            );
            let error = resolve_skill_role(&[role], "reviewer").expect_err("invalid context");
