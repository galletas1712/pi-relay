...
//! `session.drive()`. Boundary operations (compact, rewind, fork,
//! replace_transcript) live behind `SessionBoundary<'_>`, obtained via
//! `session.boundary(external_work)?`. See `rust/docs/architecture.md`.
...
pub use crate::boundary::{ExternalWork, SessionBoundary, SessionBoundaryError};
