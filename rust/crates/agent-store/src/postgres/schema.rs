...
from sessions s
where q.session_id=s.id and q.provider_config is null;
alter table queued_inputs alter column provider_config set not null;

alter table actions add column if not exists provider_config jsonb null;
update actions a
set provider_config=s.provider_config
from sessions s
where a.session_id=s.id and a.provider_config is null;
