...
async fn only_a_batch_with_no_durable_obligations_skips_the_transaction() {
    ...
    let obligations = [
        (
            "transcript entry",
            OutputBatch::new(&entries, Some("entry"), &[], &[]),
        ),
        (
            "active leaf change",
        ...
        (
            ...
            OutputBatch::new(&[], None, &events, &[]),
        ),
        ("action", OutputBatch::new(&[], None, &[], &actions)),
        (
            "action update / compaction completion",
            OutputBatch::new(&[], None, &[], &[]).with_action_update(Some(action_update())),
        ),
        (
            "consumed input",
            OutputBatch::new(&[], None, &[], &[]).with_consumed_input(Some(consumed_input())),
        ),
        (
            "accepted input",
            OutputBatch::new(&[], None, &[], &[]).with_accepted_input(Some(accepted_input())),
        ),
        (
            "provider replay attachment",
            OutputBatch::new(&[], None, &[], &[]).with_provider_replay_attachment(),
        ),
        (
            "selected-subagent control transition",
            OutputBatch::new(&[], None, &[], &[]).with_control_interrupt("input"),
        ),
    ];
