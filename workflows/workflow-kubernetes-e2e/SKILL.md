...
- tester — full subagent (runs cluster ops; writes evidence files into the
  ...
  kubernetes specifics in its stage prompt.

## Outcomes (suggested_next, in index.json)
- tester: pass | product_failure | environment_retry | human_needed

## Control flow
1. tester (a full stage; put the cluster-safety rules below in its prompt)
2. On its outcome:
   - pass             -> DONE
...
stage prompt, and REQUIRE the tester to end its final message with a line
