...
describe("Agent", () => {
	...
	it("hasQueuedMessages ignores background tool results in the mailbox", () => {
		const agent = new Agent();
		agent.mailbox.enqueue({
			kind: "tool_result",
			message: {
				role: "custom",
				...
				display: true,
				details: { toolCallId: "tool-1", toolName: "bash", isError: false },
				timestamp: Date.now(),
			},
		});
	...
	it("continue() should drain all queued steering messages from an assistant tail in one run", async () => {
		let responseCount = 0;
		const agent = new Agent({
			streamFn: () => {
				const stream = new MockAssistantStream();
	...
	it("continue() should resume from an assistant tail when background tool results are waiting in the mailbox", async () => {
		let responseCount = 0;
		const agent = new Agent({
			streamFn: () => {
				const stream = new MockAssistantStream();
		...
		expect(
			agent.state.messages.some(
				(message) =>
					...
					message.content.some((block) => block.type === "text" && block.text === "background done"),
			),
		).toBe(true);
		expect(agent.state.messages[agent.state.messages.length - 1]?.role).toBe("assistant");
	});
