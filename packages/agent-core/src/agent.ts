...
function createMutableAgentState(
	...
	return {
		...
		systemBlocks: initialState?.systemBlocks,
		messageCacheHints: initialState?.messageCacheHints,
...
export class Agent {
	...
	private readonly _mailbox: Mailbox<MailboxItem>;
	...
	public onBackgroundToolStart?: (context: BackgroundToolStartContext, signal?: AbortSignal) => Promise<void> | void;
	public onBackgroundToolEnd?: (context: BackgroundToolEndContext, signal?: AbortSignal) => Promise<void> | void;
	...
	constructor(options: AgentOptions = {}) {
		...
		this.onBackgroundToolStart = options.onBackgroundToolStart;
		this.onBackgroundToolEnd = options.onBackgroundToolEnd;
		this._mailbox = new Mailbox<MailboxItem>();
	...
	/** Public mailbox for external producers. */
	get mailbox(): Mailbox<MailboxItem> {
		return this._mailbox;
	}

	/** Compatibility accessors for session settings. */
	...
	steer(message: AgentMessage): void {
		this._mailbox.enqueue({ kind: "steering", message });
	}

	/** Queue a message to run only after the agent would otherwise stop. */
	followUp(message: AgentMessage): void {
		this._mailbox.enqueue({ kind: "follow_up", message });
	}

	/** Remove all queued steering messages. */
	clearSteeringQueue(): void {
		this._mailbox.tryDrain((item) => item.kind === "steering");
	}

	/** Remove all queued follow-up messages. */
	clearFollowUpQueue(): void {
		this._mailbox.tryDrain((item) => item.kind === "follow_up");
	}

	/** Remove all queued steering and follow-up messages. */
	clearAllQueues(): void {
		this._mailbox.tryDrain((item) => item.kind !== "tool_result");
	}

	/** Returns true when either queue still contains pending messages. */
	hasQueuedMessages(): boolean {
		return this._mailbox.hasMatching((item) => item.kind === "steering" || item.kind === "follow_up");
	}
	...
	/** Abort the current run, close the mailbox, and stop future enqueues. */
	dispose(): void {
		this.abort();
		this._mailbox.close();
	}
	...
	async continue(): Promise<void> {
		...
		if (lastMessage.role === "assistant") {
			const queuedSteering = this._mailbox.tryDrain((item) => item.kind === "steering").map((item) => item.message);
			if (queuedSteering.length > 0) {
				await this.runPromptMessages(queuedSteering);
				return;
			}

			const queuedFollowUps = this._mailbox.tryDrain((item) => item.kind === "follow_up").map((item) => item.message);
			if (queuedFollowUps.length > 0) {
				await this.runPromptMessages(queuedFollowUps);
				return;
			}

			const queuedToolResults = this._mailbox.tryDrain((item) => item.kind === "tool_result").map((item) => item.message);
			if (queuedToolResults.length > 0) {
				await this.runPromptMessages(queuedToolResults);
				return;
			}
	...
	private createContextSnapshot(): AgentContext {
		return {
			...
			systemBlocks: this._state.systemBlocks,
			messageCacheHints: this._state.messageCacheHints,
	...
	private createLoopConfig(): AgentLoopConfig {
		return {
			...
			onBackgroundToolStart: this.onBackgroundToolStart,
			onBackgroundToolEnd: this.onBackgroundToolEnd,
			convertToLlm: this.convertToLlm,
			transformContext: this.transformContext,
			getApiKey: this.getApiKey,
			mailbox: this._mailbox,
			backgroundAllowlist: this.backgroundAllowlist,
		};
	}
