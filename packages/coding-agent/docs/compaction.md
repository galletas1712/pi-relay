...
For TypeScript definitions in your project, inspect `node_modules/@pi-relay/coding-agent/dist/`.
...
```typescript
import { convertToLlm, serializeConversation } from "@pi-relay/coding-agent";
...
See [custom-compaction.ts](../../extensions/examples/custom-compaction.ts) for a complete example using a different model.
