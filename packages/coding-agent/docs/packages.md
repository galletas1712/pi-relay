...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@pi-relay/ai`, `@pi-relay/agent-core`, `@pi-relay/coding-agent`, `@pi-relay/tui`, `@sinclair/typebox`.
