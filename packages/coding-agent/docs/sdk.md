...
```bash
npm install @pi-relay/coding-agent
```
...
```typescript
import { createAgentSession } from "@pi-relay/coding-agent";
...
The `Agent` class (from `@pi-relay/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { AuthStorage, ModelRegistry } from "@pi-relay/coding-agent";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@pi-relay/coding-agent";
...
import { createAgentSession, defineTool } from "@pi-relay/coding-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@pi-relay/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@pi-relay/coding-agent";
...
See [RPC wire schema](../src/client/rpc/wire.ts) for the JSON protocol.
