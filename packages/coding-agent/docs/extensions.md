...
See [extensions/examples/](../../extensions/examples/) for working implementations.
...
| Package | Purpose |
|---------|---------|
| `@pi-relay/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `@sinclair/typebox` | Schema definitions for tool parameters |
| `@pi-relay/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@pi-relay/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@pi-relay/coding-agent";
...
```typescript
import { isBashToolResult } from "@pi-relay/coding-agent";
...
```typescript
import { createLocalBashOperations } from "@pi-relay/coding-agent";
...
Transforms chain across handlers. See [input-transform.ts](../../extensions/examples/input-transform.ts).
...
`false` in print mode (`-p`) and JSON mode. `true` in interactive and RPC mode. In RPC mode, dialog methods (`select`, `confirm`, `input`, `editor`) work via the extension UI sub-protocol, and fire-and-forget methods (`notify`, `setStatus`, `setWidget`, `setTitle`, `setEditorText`) emit requests to the client. Some TUI-specific methods are no-ops or return defaults (see [RPC wire schema](../src/client/rpc/wire.ts)).
...
```typescript
import { SessionManager } from "@pi-relay/coding-agent";
...
See [dynamic-tools.ts](../../extensions/examples/dynamic-tools.ts) for a full example.
...
See [send-user-message.ts](../../extensions/examples/send-user-message.ts) for a complete example.
...
```typescript
import type { AutocompleteItem } from "@pi-relay/tui";
...
```typescript
import { withFileMutationQueue } from "@pi-relay/coding-agent";
...
**Important:** Use `StringEnum` from `@pi-relay/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
See [extensions/examples/tool-override.ts](../../extensions/examples/tool-override.ts) for a complete example that overrides `read` with logging and access control.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@pi-relay/coding-agent";
...
```typescript
import { createBashTool } from "@pi-relay/coding-agent";
...
See [extensions/examples/ssh.ts](../../extensions/examples/ssh.ts) for a complete SSH example with `--ssh` flag.
...
See [extensions/examples/truncated-tool.ts](../../extensions/examples/truncated-tool.ts) for a complete example wrapping `rg` (ripgrep) with proper truncation.
...
```typescript
import { keyHint } from "@pi-relay/coding-agent";
...
See [extensions/examples/timed-confirm.ts](../../extensions/examples/timed-confirm.ts) for complete examples.
...
```typescript
import { Text, Component } from "@pi-relay/tui";
...
See [tui.md](tui.md) for the full `OverlayOptions` API and [overlay-qa-tests.ts](../../extensions/examples/overlay-qa-tests.ts) for examples.
...
import { matchesKey } from "@pi-relay/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@pi-relay/coding-agent";
...
| RPC (`--mode rpc`) | JSON protocol | Host handles UI, see [wire.ts](../src/client/rpc/wire.ts) |
...
All examples in [extensions/examples/](../../extensions/examples/).
