...
The web client serializes effort writes independently per session. Rapid edits
coalesce to the latest desired value, successful responses patch the captured
session's list and warm snapshot cache even after navigation, and a final
failure clears the optimistic value, shows a persistent actionable notice, and
refetches the selected session. Connection recovery and ordinary snapshot/list
refetches remain authoritative and converge on the daemon's persisted default.
