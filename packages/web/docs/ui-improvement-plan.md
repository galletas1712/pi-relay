...
| Existing safe areas and consistent input typography | The mobile top bar, left drawer, right Inspector, notices, and composer apply the relevant safe-area inset (`packages/web/src/styles.css:243-255`, `packages/web/src/styles.css:328-332`, `packages/web/src/styles.css:2980-2984`, `packages/web/src/styles.css:3599-3604`, `packages/web/src/styles.css:3755-3757`). Inputs and the composer use the shared `--text-lg` size across breakpoints (`packages/web/src/styles.css:243-251`, `packages/web/src/styles.css:3427-3452`). | Extend, do not regress, these protections or introduce breakpoint-specific input font sizes. |
...
- Add one actions Menu exposing **History**, **Export**, and **Context Summary** where current data supports it.
- Keep **System prompt** at the durable transcript start rather than duplicating it in the actions Menu.
