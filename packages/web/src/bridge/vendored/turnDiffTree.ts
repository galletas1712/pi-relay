...
// Source: apps/web/src/lib/turnDiffTree.ts
// Local adaptation: TurnDiffFileChange is a local struct.
// Local struct replacing T3's TurnDiffFileChange type: the fields this
// module reads.
...
function toTreeNodes(directory: MutableDirectoryNode): TurnDiffTreeNode[] {
  const subdirectories: TurnDiffTreeDirectoryNode[] = Array.from(directory.directories.values())
    // vendored adaptation: [...].sort instead of toSorted (app tsconfig lib < es2023)
    .sort(compareByName)
    .map<TurnDiffTreeDirectoryNode>((subdirectory: MutableDirectoryNode) => ({
      kind: "directory",
