...
```bash
npm install @pi-relay/ai
```

TypeBox exports are re-exported from `@pi-relay/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@pi-relay/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@pi-relay/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@pi-relay/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@pi-relay/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@pi-relay/ai';
...
```typescript
import { getModel, stream } from '@pi-relay/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@pi-relay/ai';
...
```typescript
import { Model, stream } from '@pi-relay/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@pi-relay/ai';
...
```typescript
import { getModel, complete, Context } from '@pi-relay/ai';
...
```typescript
import { Context, getModel, complete } from '@pi-relay/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@pi-relay/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@pi-relay/ai';
...
- **Anthropic** (Claude Pro/Max subscription or Anthropic Console API billing)
...
```bash
npx @pi-relay/ai login              # interactive provider selection
npx @pi-relay/ai login anthropic    # login to specific provider
npx @pi-relay/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@pi-relay/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@pi-relay/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@pi-relay/ai/oauth';
...
```typescript
import { getModel, complete } from '@pi-relay/ai';
import { getOAuthApiKey } from '@pi-relay/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@pi-relay/ai`
